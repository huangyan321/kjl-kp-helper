...
async function doSwitchBranch(branchName: string, repoUrl?: string): Promise<void> {
  await window.withProgress(
    { location: ProgressLocation.Notification, title: `切换分支: ${branchName}`, cancellable: false },
    async () => {
      await gitSwitchBranch(branchName, repoUrl)
    },
  )
  window.showInformationMessage(`已切换到 ${branchName}`)
